...
%corr=smooth(novatel_wz_ss(1:30000),100);

[corr_db3, s, b]=wden(novatel_wz_ss(1:30000), 'rigrsure', 's', 'mln', 4, 'db3');
[corr_db5, s, b]=wden(novatel_wz_ss(1:30000), 'rigrsure', 's', 'mln', 4, 'db5');
[corr_db8, s, b]=wden(novatel_wz_ss(1:30000), 'rigrsure', 's', 'mln', 4, 'db8');
ac_xbow_wz_ss_db3 = xcorr2(corr_db3);
ac_xbow_wz_ss_db5 = xcorr2(corr_db5);
ac_xbow_wz_ss_db8 = xcorr2(corr_db8);
x_ax = (-30000/200+1/200):1/200:(30000/200 - 1/200);
plot(x_ax,ac_xbow_wz_ss_db3/max(ac_xbow_wz_ss_db3));hold on; plot(x_ax,ac_xbow_wz_ss_db5/max(ac_xbow_wz_ss_db5),'r');hold on;plot(x_ax,ac_xbow_wz_ss_db8/max(ac_xbow_wz_ss_db8),'g');
lg=legend('db3','db5','db8');
    gt1=findobj(lg,'type','text');
    set(gt1,'fontname','--','fontweight','bold');
    
    xlabel('time (seconds)','fontweight','bold','fontsize',12);
    ylabel('Autocorrelation function','fontweight','bold','fontsize',12);
    title('Comparison of ACF for different wavelets functions: Xbow Gyro Z ','fontweight','bold','fontsize',16)
plot(novatel_wz_ss(1:30000)); hold on;
plot(corr, 'r');
t=1;
...
% 
% subplot(2,1,1)
% plot(x_axis,smooth_xbow_wz_ss_50,'b'); hold on; plot(smooth_xbow_wz_ss_200, 'r'); plot(smooth_xbow_wz_ss_400, 'g');hold on; plot(smooth_xbow_wz_ss_1000,'y');
%      lg=legend('Span=0.25 s','Span=1 s','Span=2 s','Span=5 s');
%     gt1=findobj(lg,'type','text');
%     set(gt1,'fontname','--','fontweight','bold');
%     
%     xlabel('time (seconds)','fontweight','bold','fontsize',12);
%     ylabel('Xbow Gyro-Z Axis Data (deg/sec)','fontweight','bold','fontsize',12);
%     title('Xbow Gyro denoising with moving average','fontweight','bold','fontsize',16)
%     
%     x_a = [0.25 0.5 1 2 3 5 8 10];
%     subplot(2,1,2)
%     plot(x_a, std_smooth,'r*');
%     xlabel('MA span (seconds)','fontweight','bold','fontsize',12);
%     ylabel('Standard deviation - Xbow Gyro Z axis (deg/sec)','fontweight','bold','fontsize',12);
%     title('Standad deviation variation wrt MA span','fontweight','bold','fontsize',16)
